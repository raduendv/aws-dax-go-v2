...
const (
	...
	daxConnectionsPendingAcquire  = "dax.connections.pending_acquire" // gauge
	...
	daxRouteManagerDisabledState  = "dax.route_manager.disabled_state" // gauge

	//Status of the manager (0=enabled, 1=disabled).
	daxRouteManagerStateEnabled  = 0
	daxRouteManagerStateDisabled = 1
)
...
func buildGauges(meter metrics.Meter, om *daxSdkMetrics, ops []string) (err error) {
	gauges := map[string]string{
		...
		daxConnectionsPendingAcquire: "Current number of concurrent connection attempts",
		daxRouteManagerDisabledState: "Status of the manager (0=enabled, 1=disabled).",
	}
...
func buildDaxSdkMetrics(mp metrics.MeterProvider) (*daxSdkMetrics, error) {
	...
	ops := []string{
		opDefineAttributeList,
		opDefineAttributeListId,
		opDefineKeySchema,
		opEndpoints,
		OpGetItem,
		OpPutItem,
	...
	if err := buildCounters(meter, om, ops); err != nil {
		return nil, err
	}

	if err := buildHistograms(meter, om, ops); err != nil {
		return nil, err
	}

	if err := buildGauges(meter, om, ops); err != nil {
		return nil, err
	}
