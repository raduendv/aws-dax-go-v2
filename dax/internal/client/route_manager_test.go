...
func Test_verifyAndDisable(t *testing.T) {
	...
	expectGauges(t, om, map[string]int{
		daxRouteManagerDisabledState: daxRouteManagerStateDisabled,
	})
	...
	expectGauges(t, om, map[string]int{
		daxRouteManagerDisabledState: daxRouteManagerStateEnabled,
	})
...
func Test_daxConnectionsIdle(t *testing.T) {
	...
	expectGauges(t, om, map[string]int{
		daxConnectionsPendingAcquire: 20,
	})

	<-time.After(time.Second)

	expectGauges(t, om, map[string]int{
		daxConnectionsPendingAcquire: 0,
	})
}
