...
func TestDeserializationError(t *testing.T) {
	// Test case 1: Basic deserialization error
	t.Run("Basic deserialization error", func(t *testing.T) {
		err := &smithy.DeserializationError{
			Err: fmt.Errorf("failed to unmarshal data"),
		}

		expectedMsg := "deserialization failed, failed to unmarshal data"
		if err.Error() != expectedMsg {
			t.Errorf("expected '%s', got '%s'", expectedMsg, err.Error())
		}
	})

	// Test case 2: Deserialization error with wrapped error
	t.Run("Wrapped deserialization error", func(t *testing.T) {
		originalErr := fmt.Errorf("original error")
		err := &smithy.DeserializationError{
			Err: fmt.Errorf("failed to unmarshal: %w", originalErr),
		}
	...
	// Test case 3: Check if error is of type DeserializationError
	t.Run("Error type assertion", func(t *testing.T) {
		err := &smithy.DeserializationError{
			Err: fmt.Errorf("some error"),
		}

		_, ok := interface{}(err).(*smithy.DeserializationError)
		if !ok {
			t.Error("expected error to be of type *smithy.DeserializationError")
		}
	})
}
...
func TestMalformedCBORString(t *testing.T) {
	invalidCBOR := []byte{0x78, 0x02, 0x41} // Declared length is 2, but only 1 byte provided
	r := NewReader(bytes.NewReader(invalidCBOR))

	_, err := r.ReadString()
	if err == nil {
		t.Errorf("Expected error for malformed CBOR string, got nil")
	}
}

func TestExceedMaxObjLen(t *testing.T) {
	oversizedCBOR := []byte{0x5a, 0x40, 0x00, 0x00, 0x01} // Bytes header with length > maxObjLenBytes
	r := NewReader(bytes.NewReader(oversizedCBOR))

	_, err := r.ReadBytes()
	if err != ErrObjTooBig {
		t.Errorf("Expected ErrObjTooBig, got %v", err)
	}
}

func TestTruncatedFloat64(t *testing.T) {
	truncatedFloatCBOR := []byte{0xfb, 0x40, 0x09, 0x21} // Incomplete 64-bit float
	r := NewReader(bytes.NewReader(truncatedFloatCBOR))

	_, err := r.ReadFloat64()
	if err == nil {
		t.Errorf("Expected error for truncated float64, got nil")
	}
}
